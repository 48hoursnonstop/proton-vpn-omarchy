...
// PanelHero-compatible header with a wrapping description. Omarchy's stock
// component deliberately elides metadata to one line, which is too restrictive
// for onboarding copy and translated strings.
Item {
  id: root
    ...
    Row {
      ...
      Text {
        ...
        width: Math.min(
          ...
          Math.max(
            0,
            parent.width - (detailPill.visible
      ...
      Item {
        width: Math.max(
          0,
          parent.width - titleText.width - detailPill.implicitWidth
        )
        height: 1
      }
