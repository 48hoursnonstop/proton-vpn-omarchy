...
Item {
  ...
  property string settingsSection: 'connection'
  ...
  function captureViewState() {
    return { section: settingsSection, advanced: advancedExpanded }
  }

  function restoreViewState(state) {
    if (!state) return
    settingsSection = state.section === 'application' ? 'application' : 'connection'
    advancedExpanded = !!state.advanced
  }
  ...
  Column {
    ...
    RowLayout {
      ...
      Repeater {
        ...
        delegate: ProtonButton {
          ...
          selected: root.settingsSection === modelData
          foreground: root.foreground
          Accessible.checkable: true
          Accessible.checked: selected
          onClicked: { root.settingsSection = modelData; root.openPicker = '' }
        }
      }
    }
    Column {
      ...
      PanelActionRow {
        ...
        rowForeground: root.foreground
        rowFontFamily: root.fontFamily
        title: root.label('advanced_network')
        iconName: 'sliders'
        detailIconName: 'chevron_right'
        detailIconRotation: root.advancedExpanded ? 90 : 0
        Accessible.checkable: true
        Accessible.checked: root.advancedExpanded
        onActivated: root.advancedExpanded = !root.advancedExpanded
      }
      Column {
        width: parent.width
        spacing: Style.space(8)
    ...
    Column {
      width: parent.width
      spacing: Style.space(8)
      visible: root.settingsSection === 'application'
      PanelActionRow {
        width: parent.width
        rowForeground: root.foreground
        rowFontFamily: root.fontFamily
      ...
      ProtonButton {
        width: parent.width
        label: root.confirmingLogout ? root.label('confirm_sign_out') : root.label('sign_out')
